...
- Standing rules: PR into `main`; no VERSION/CHANGELOG; `npm test` + `npm run check` green; remix:check untouched. Dev-PC verification (desktop browser, DevTools device emulation for orientation); real-device pass via the beta channel afterwards.
